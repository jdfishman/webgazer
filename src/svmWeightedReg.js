...
(function(window) {
     ...
     * @constructor
     */
    webgazer.reg.svmWeightedReg = function() {
     ...
     */
    webgazer.reg.svmWeightedReg.prototype.addData = function(eyes, screenPos, type) {
        if (!eyes) {
            return;
        }
    ...
    webgazer.reg.svmWeightedReg.prototype.predict = function(eyesObj) {
        ...
        var eyeFeats = getEyeFeats(eyesObj);
        eyeFeats.push(1);
        var predictedX = svmPredict(eyeFeats, x_svm);
        /*for(var i=0; i< eyeFeats.length; i++){
            predictedX += eyeFeats[i] * coefficientsX[i];
        }*/
        var predictedY = svmPredict(eyeFeats, y_svm);
        /*for(var i=0; i< eyeFeats.length; i++){
            predictedY += eyeFeats[i] * coefficientsY[i];
        }*/
    ...
    function svmPredict(x, svm) {
        var prediction = svm.bias;
        for (i  = 0; i < svm.support_vectors.length; i++) {
            prediction += svm.support_weights[i]*kernelCompute(x, svm.support_vectors[i], x.length);
        }
    }
     ...
     */
    webgazer.reg.svmWeightedReg.prototype.setData = function(data) {
     ...
     */
    webgazer.reg.svmWeightedReg.prototype.getData = function() {
     ...
     * @type {string}
     */
    webgazer.reg.svmWeightedReg.prototype.name = 'ridge';
