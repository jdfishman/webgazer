...
(function(window) {
    ...
    var x_svm_url = "http://localhost:3000/x_svm";
    var y_svm_url = "http://localhost:3001/y_svm";
    var x_bias = null;
    var y_bias = null;
    var x_weights = null;
    var y_weights = null;
     ...
     * @constructor
     */
    webgazer.reg.asyncSvmWeightedReg = function() {
        ...
        //console.log(x_svm); //bias, support_vectors, support_weights
        //console.log(y_svm);
        //load_svms();
    };

    function load_svms() {
        $.get(x_svm_url, function(data){
            x_bias = data.bias;
            x_weights = data.support_weights;
            console.log('loaded x');
        });

        $.get(y_svm_url, function(data){
            y_bias = data.bias;
            y_weights = data.support_weights;
     ...
     */
    webgazer.reg.asyncSvmWeightedReg.prototype.addData = function(eyes, screenPos, type) {
        if (!eyes) {
            return;
        }
     ...
     * @returns {Object}
     */
    /**webgazer.reg.svmWeightedReg.prototype.predict = function(eyesObj) {
        if (!eyesObj) {
            return null;
        }
    ...
    webgazer.reg.asyncSvmWeightedReg.prototype.predict = function(eyesObj) {
        if (!eyesObj) {
            return null;
        }
        /*if (x_bias == null || y_bias == null || x_weights == null || y_weights == null) {
            return null;
        }*/

        var eyeFeats = getEyeFeats(eyesObj);
        if (eyeFeats == null || eyeFeats == undefined) {
            return null;
        }
        eyeFeats.push(1);
        //var kernelX = getKernel(eyeFeats, eyeFeats.length, x_svm_url);
        //var predictedX = svmPredict(eyeFeats, x_bias, x_weights, kernelX);
        predictX(eyeFeats, x_svm_url);
        predictY(eyeFeats, y_svm_url);
        while (this.predictedX == null || this.predictedY == null) {
        }
        /*for(var i=0; i< eyeFeats.length; i++){
            predictedX += eyeFeats[i] * coefficientsX[i];
        }*/
        //var kernelY = getKernel(eyeFeats, eyeFeats.length, y_svm_url);
        //var predictedY = svmPredict(eyeFeats, y_bias, y_weights, kernelY);
        ...
        var prediction = {
            x: Math.floor(this.predictedX),
            y: Math.floor(this.predictedY)
        };
    ...
    function predictX(x, svm_url) {
        $.get(svm_url, function(data){
                var prediction = data.bias;
                for (i = 0; i < x.length; i++) {
                    var kernel = 0;
                    for (j = 0; j < data.support_vectors.length; j++) {
                        kernel += kernelCompute(x, data.support_vectors[j], x.length);
                    }
                    prediction += weights[i]*kernel;
                }
                this.predictedX = prediction;
            });
    }

    function predictY(y, svm_url) {
        $.get(svm_url, function(data){
                var prediction = data.bias;
                for (i = 0; i < x.length; i++) {
                    var kernel = 0;
                ...
                this.predictedY = prediction;
            });
    }

    function compute(x, svm_url) {
    ...
    //x_svm : bias, support_vectors, support_weights
    //n - number of support vectors
    //d - feature length of support vectors
    function svmPredict(x, bias, weights, kernel) {
        var prediction = bias;
        for (i  = 0; i < weights.length; i++) {
            prediction += weights[i]*kernel[i];
        }
    }

    function kernelCompute(x, y, gamma) {
        var distance = 0; //euclidean distance squared
        for (i = 0; i < x.length; i++) {
            distance += (x[i]-y[i])*(x[i]-y[i]);
        }
        return Math.exp(-distance/gamma);
    }
     ...
     */
    webgazer.reg.asyncSvmWeightedReg.prototype.setData = function(data) {
     ...
     */
    webgazer.reg.asyncSvmWeightedReg.prototype.getData = function() {
     ...
     * @type {string}
     */
    webgazer.reg.asyncSvmWeightedReg.prototype.name = 'ridge';
